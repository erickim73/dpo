...
for gamma in gammas:
    ...
    print(f'\nTraining {method} on {env_name} with gamma {gamma}')
    train_benchmark_model(method, gamma, env_name, total_samples=int(total_samples_dict[env_name]), 
                          lr=3e-4, log_interval=500)
    print('Training takes {:.3f} hours'.format((time.time() - individual_start_time) / 3600))

print('\n\nTotal training takes {:.3f} hours'.format((time.time() - start_time) / 3600))
